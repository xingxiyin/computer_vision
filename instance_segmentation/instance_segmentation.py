...
# Loading he COCO class labels our Mask R-CNN was trained on
labelsPath = os.path.sep.join([args["mask_rcnn"], "object_detection_classes_coco.txt"])
LABELS = open(labelsPath).read().strip().split("\n")


# Derive the path to the Mask R-CNN weights and model configuration
...
while True:
    ...
    # Sort the indexes of the bounding boxes in by their corresponding prediction probability (in descending order)
    idxs = np.argsort(boxes[0, 0, :, 2])[::-1]

    # Initialize the mask, ROI, and coordinates of the person for the current frame
    mask = None
    roi = None
    coords = None
